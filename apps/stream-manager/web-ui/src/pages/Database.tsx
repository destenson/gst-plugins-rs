...
import type { DatabaseRecord, TableInfo } from "../api/services/DatabaseAPI.ts";
...
const Database: React.FC = () => {
  ...
  const handleClearTable = async () => {
    if (
      !confirm(
        `Are you sure you want to clear all data from ${selectedTable}? This cannot be undone.`,
      )
    ) return;
                                  ...
                                  <td key={key} className="px-6 py-4 whitespace-nowrap">
                                    {editingRow === index
                                      ...
                                      : (
                                        <span
                                          className={`text-sm ${
                                            getColumnType(value) === "datetime"
                                              ? "font-mono text-xs"
                                              : getColumnType(value) === "url"
                                              ? "text-blue-600 dark:text-blue-400"
                                              : getColumnType(value) === "boolean"
                                              ? "font-semibold"
