...
import type { DetailedMetrics, Recording, Stream } from "../api/types/index.ts";
...
function VideoPlayer(
  { streamId, sourceUrl, status }: { streamId: string; sourceUrl: string; status: string },
) {
...
export default function StreamDetail() {
  ...
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<"overview" | "recordings" | "config" | "events">(
    "overview",
  );
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-4">
          The stream "{id}" could not be found.
        </p>
        <Link
          to="/streams"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <InfoCard
                  ...
                  icon={Wifi}
                  trend={stream.metrics?.packets_lost
                ...
                <InfoCard
                  ...
                  value={stream.last_connected
                    ? `${
                      Math.floor(
                        (Date.now() - new Date(stream.last_connected).getTime()) / (1000 * 60),
                      )
                    }m`
                    : "N/A"}
                  icon={Clock}
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                ...
                <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <dd className="text-sm font-medium">
                      {stream.last_connected
                        ? format(new Date(stream.last_connected), "PPpp")
                        : "Never"}
                    </dd>
                  </div>
                  <div>
                    ...
                    {recordings.map((recording) => (
                      <RecordingItem
                        ...
                        onDownload={() => {
                          ...
                          globalThis.open(
                            `http://localhost:8080/recordings/${recording.filename}/download`,
                            "_blank",
                          );
                        }}
                        onDelete={async () => {
