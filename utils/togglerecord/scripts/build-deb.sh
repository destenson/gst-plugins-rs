...
ELEMENT_DIR="$(dirname "$SCRIPT_DIR")"
PROJECT_ROOT="$(dirname "$(dirname "$ELEMENT_DIR")")"
...
# Configuration
PACKAGE_NAME="gst-plugin-togglerecord"
...
# Build the plugin
build_plugin() {
    log_info "Building gst-plugin-togglerecord (release mode)..."
    cd "$ELEMENT_DIR"
    ...
    local lib_file
    if [ "$(uname)" = "Linux" ]; then
        lib_file="${PROJECT_ROOT}/target/release/libgsttogglerecord.so"
    else
        lib_file="${PROJECT_ROOT}/target/release/gsttogglerecord.dll"
    fi
...
main() {
    ...
    log_info "Working directory: $ELEMENT_DIR"
...
Build Debian package for gst-plugin-togglerecord
