#!/bin/bash
# Test script for validating gst-plugin-togglerecord Debian package
...
ldd /usr/lib/*/gstreamer-1.0/libgsttogglerecord.so | head -10
...
dpkg -r gst-plugin-togglerecord
# Verify plugin is no longer available
if gst-inspect-1.0 togglerecord 2>/dev/null; then
...
# Test purge
dpkg -i /workspace/*.deb
dpkg -P gst-plugin-togglerecord
...
test_local_install() {
    ...
    sudo dpkg -r gst-plugin-togglerecord
