...
echo -e "${GREEN}Building gst-plugin-livesync Debian package...${NC}"
...
if [ "$VERBOSE" = true ]; then
    cargo build -p gst-plugin-livesync --release $BUILD_FLAGS --verbose
else
    cargo build -p gst-plugin-livesync --release $BUILD_FLAGS
fi

# Verify the library was built
if [ ! -f "target/release/libgstlivesync.so" ]; then
