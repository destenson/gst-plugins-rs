#!/bin/bash
# Test script for validating gst-plugin-inter Debian package
...
ldd /usr/lib/*/gstreamer-1.0/libgstrsinter.so | head -10
...
dpkg -r gst-plugin-inter
# Verify plugin is no longer available
if gst-inspect-1.0 inter 2>/dev/null; then
