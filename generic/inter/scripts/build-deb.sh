...
# Build the plugin
build_plugin() {
    log_info "Building gst-plugin-inter (release mode)..."
    cd "$ELEMENT_DIR"
    ...
    local lib_file
    if [ "$(uname)" = "Linux" ]; then
        lib_file="${PROJECT_ROOT}/target/release/libgstrsinter.so"
    else
        lib_file="${PROJECT_ROOT}/target/release/gstinter.dll"
    fi
...
Build Debian package for gst-plugin-inter
