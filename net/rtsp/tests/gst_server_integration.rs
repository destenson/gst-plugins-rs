...
use gst::prelude::*;
use rtsp_test_server::{GstRtspTestServer, ServerType, ServerConfig, helpers};
...
fn test_authentication_with_real_server() {
    ...
    // Test without credentials (should fail)
    {
        ...
        assert!(error_received.load(Ordering::SeqCst), "Expected authentication error");
    ...
    // Test with correct credentials (should succeed)
    {
        ...
        let (state_result, _, _) = pipeline.state(Some(gst::ClockTime::from_seconds(1)));
        assert_eq!(state_result, Ok(gst::StateChangeSuccess::Success), "Should authenticate successfully");
...
fn test_audio_video_stream_with_real_server() {
    ...
    pipeline.add_many(&[&src, &video_sink, &audio_sink]).unwrap();
    ...
    assert!(video_connected.load(Ordering::SeqCst), "Video stream should be connected");
    assert!(audio_connected.load(Ordering::SeqCst), "Audio stream should be connected");
...
fn test_reconnection_with_real_server() {
    ...
    let (state_result, _, _) = pipeline.state(Some(gst::ClockTime::from_seconds(1)));
    assert_eq!(state_result, Ok(gst::StateChangeSuccess::Success), "Should reconnect after server restart");
