...
mod tests {
    ...
    fn test_connection_limited_device_auto_mode() {
        ...
        let racing_strategy = harness
            ...
            .unwrap_or_default();
        assert_eq!(
            racing_strategy, "last-wins",
            "Auto mode should switch to last-wins for connection-limited devices"
        );
    }
    ...
    fn test_connection_limited_retry_behavior() {
        ...
        harness
            .set_property("retry-initial-delay", 1000u32)
            .unwrap(); // 1 second
        ...
        assert!(
            ...
            "Should have at least 1 connection failure"
        );
    ...
    fn test_connection_limited_with_keepalive() {
        ...
        assert!(
            harness.is_playing(),
            "Should still be playing with keep-alive"
        );

        let stats = harness.get_retry_stats().unwrap();
        assert_eq!(
            stats.connection_attempts, 1,
            "Should maintain single connection with keep-alive"
        );
    }
}
