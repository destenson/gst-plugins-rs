...
mod tests {
    ...
    fn test_http_tunneling_activation() {
        ...
        let mut harness = RtspTestHarness::new(&tunnel_url)
    ...
    fn test_http_tunneling_fallback() {
        ...
        assert!(stats.connection_attempts >= 2, "Should have multiple attempts (TCP then HTTP)");
    }
    ...
    fn test_http_tunneling_with_proxy() {
        ...
        harness.set_property("proxy", "http://proxy.example.com:8080").unwrap();
        ...
        assert!(proxy_attempted || !harness.is_playing(), "Should attempt proxy connection");
    }
    ...
    fn test_http_tunneling_performance() {
        ...
        assert!(connected, "Should connect via HTTP tunnel");
        assert!(connect_time < Duration::from_secs(10), "HTTP tunnel connection should be reasonably fast");
        ...
        assert!(data_received || harness.is_playing(), "Should receive data through HTTP tunnel");
    }
    ...
    fn test_http_tunneling_reconnection() {
        ...
        let stats = harness.get_retry_stats().unwrap();
        assert!(stats.connection_successes >= 2, "Should have at least 2 successful connections");
    }
}