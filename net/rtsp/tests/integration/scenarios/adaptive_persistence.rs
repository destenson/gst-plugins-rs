...
mod tests {
    ...
    fn test_adaptive_learning_persistence() {
        ...
        // First session - train the adaptive system
        {
            ...
            assert!(!racing_strategy.is_empty(), "Should have determined a racing strategy");
        ...
        // Second session - verify persistence
        {
            ...
            let racing_strategy = harness.get_property::<String>("racing-strategy").unwrap_or_default();
            assert!(!racing_strategy.is_empty(), "Should have loaded persisted strategy");
    ...
    fn test_adaptive_learning_per_server() {
        ...
        // Train on first URL
        {
            let mut harness = RtspTestHarness::new(&url1)
                .expect("Failed to create test harness");
            ...
            let strategy1 = harness.get_property::<String>("racing-strategy").unwrap_or_default();
            
            harness.stop().unwrap();
            
            // Now test with faulty server
            let mut harness2 = RtspTestHarness::new(&url2)
                .expect("Failed to create test harness");
            ...
            let strategy2 = harness2.get_property::<String>("racing-strategy").unwrap_or_default();
            
            // Strategies might be different based on server behavior
            println!("Strategy for normal: {}, Strategy for faulty: {}", strategy1, strategy2);
        }
        
        // Verify both are persisted
        {
            let mut harness1 = RtspTestHarness::new(&url1)
                .expect("Failed to create test harness");
            ...
            let loaded_strategy1 = harness1.get_property::<String>("racing-strategy").unwrap_or_default();
            assert!(!loaded_strategy1.is_empty(), "Should have persisted strategy for URL1");
        }
    }
    ...
    fn test_adaptive_cache_cleanup() {
        ...
        if cache_dir.exists() {
            ...
            assert!(entries.len() < 10, "Cache cleanup should limit number of entries");
        }
    }
    ...
    fn test_adaptive_learning_improves_performance() {
        ...
        let initial_connect_time = {
            ...
            harness.start().unwrap();
            harness.wait_for_connection(Duration::from_secs(30)).unwrap();
            start.elapsed()
        };
        ...
        println!("Initial connect time: {:?}, Learned connect time: {:?}", 
                 initial_connect_time, learned_connect_time);
        
        // Learned strategy should be faster or at least not significantly slower
        assert!(learned_connect_time <= initial_connect_time + Duration::from_secs(2),
