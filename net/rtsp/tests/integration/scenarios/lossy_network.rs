...
mod tests {
    ...
    fn test_lossy_network_auto_mode() {
        ...
        if stats.connection_failures > 3 {
            ...
            let racing_strategy = harness
                ...
                .unwrap_or_default();
            assert_eq!(
                racing_strategy, "first-wins",
                "Auto mode should switch to first-wins for lossy networks"
            );
        }
    }
    ...
    fn test_lossy_network_retry_resilience() {
        ...
        let stats = harness.get_retry_stats().unwrap();
        println!(
            "Lossy network stats: attempts={}, failures={}, successes={}",
    ...
    fn test_lossy_network_with_racing() {
        ...
        assert!(
            ...
            "Racing should connect faster than serial attempts"
        );
        ...
        assert!(
            racing_attempts > 0,
            "Should see evidence of connection racing"
        );
    }
    ...
    fn test_lossy_network_buffering() {
        ...
        assert!(
            underruns < 5,
            "Buffering should minimize underruns in lossy conditions"
        );
    }
}
