...
impl RtspTestHarness {
    ...
    pub fn set_property<V: ToValue>(&self, name: &str, value: V) -> Result<(), Box<dyn std::error::Error>> {
        self.rtspsrc.set_property(name, value.to_value());
        Ok(())
    }
    
    /// Get a property from the rtspsrc element
    pub fn get_property<T: for<'a> FromValue<'a> + 'static>(&self, name: &str) -> Result<T, Box<dyn std::error::Error>> {
    ...
    pub fn wait_for_state(&self, state: gst::State, timeout: Duration) -> Result<(), Box<dyn std::error::Error>> {
        let (success, _, _) = self.pipeline.state(Some(gst::ClockTime::from_seconds(timeout.as_secs())));
    ...
    pub fn wait_for_connection(&self, timeout: Duration) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    pub fn is_playing(&self) -> bool {
        let (_, current, _) = self.pipeline.state(Some(gst::ClockTime::from_mseconds(100)));
    ...
    pub fn simulate_network_condition(&self, condition: NetworkCondition) -> Result<(), Box<dyn std::error::Error>> {
        match condition {
