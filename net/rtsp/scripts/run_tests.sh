#!/bin/bash
# RTSP Test Suite - Automated validation for rtspsrc2
# 
...
# echo "RTSP Test Suite - Starting at $TIMESTAMP"
...
# Build the rtsp plugin
build_plugin() {
    log_info "Building gst-plugin-rtsp..."
    cd "$PROJECT_ROOT"
    cargo build -p gst-plugin-rtsp 2>&1 | tee "${RESULTS_DIR}/build-${TIMESTAMP}.log"
...
test_stream_isolation() {
    ...
    # Block stream 2 with iptables
    log_info "Blocking stream 2..."
    sudo iptables -A INPUT -s 127.0.0.2 -p udp -j DROP 2>/dev/null || true
    sudo iptables -A OUTPUT -d 127.0.0.2 -p udp -j DROP 2>/dev/null || true
...
UTILITIES:
  setup                     Setup test environment (build + start mediamtx)
  cleanup                   Stop all test processes and clean up
  results                   Show recent test results
  build                     Build the rtsp plugin
